...
{
    sealed partial class PartyPokerSpanLineCatParserImpl : HandHistoryParserLineCatImpl
    {
        ...
        protected override DateTime ParseDateUtc(List<string> header)
        {
            ...
            ReadOnlySpan<char> line = header[1];

            //int splitIndex = line.IndexOf(" - ") + 3;
            SpanParser date = new SpanParser(line.Slice(line.IndexOf(" - ") + 3));

            date.AdvanceTo(',', 2);
            var month = date.ReadString(date.IndexOf(' '), offset: 1);
            var day = date.ReadInt(date.IndexOf(','), offset: 2);
            var timeStr = date.Read(date.IndexOf(' '), offset: 1);
            var timezone = date.ReadString(date.IndexOf(' '), offset: 1);
            int year = int.Parse(date.Current);
        ...
        private static GameType ParseCashGameGametype(List<string> header)
        {
            ...
            ReadOnlySpan<char> game = line.Slice(line.IndexOf(' '));
            game.AdvanceTo(' ', 1);
            game.AdvanceTo(' ', 1);
            var limit = game.ReadString(2);
            game.Advance(1);
            var gameLength = game.Read(game.IndexOf(' ')).Length;
            ...
            throw new UnrecognizedGameTypeException(line.ToString(), "Unrecognized game-type: " + game.ToString());
        }
        ...
        static HandAction ParseBlindAction(ReadOnlySpan<char> line)
        {
            string playerName;
            ...
            switch (blindIdentifier)
            {
                ...
                case 'i':
                    playerName = line.Remove(amountStartIndex - 17).ToString();//" posts big blind ".Length
                    action = HandActionType.BIG_BLIND;
                    break;

                //"Player posts small blind [$5 USD]."
                case 'l':
                    playerName = line.Remove(amountStartIndex - 19).ToString();//" posts small blind ".Length
                ...
                case 'd':
                    playerName = line.Remove(amountStartIndex - 24).ToString();//" posts big blind + dead ".Length
                    action = HandActionType.POSTS;
                    break;

                //saboniiplz posts ante [400]
                case 's':
                    playerName = line.Remove(amountStartIndex - 12).ToString();//" posts big blind + dead ".Length
                    action = HandActionType.ANTE;
                    break;
                default:
                    throw new ArgumentException("Unknown posting Action: " + line.ToString());
            }

            var amount = ParseDecimal(line.Slice(amountStartIndex + 1));
        ...
        public static void ParseRegularActionLine(ReadOnlySpan<char> line, ref Street currentStreet, List<HandAction> actions, List<WinningsAction> winners)
        {
            char lastChar = line.GetLast();
            switch (lastChar)
            {
                //Expected formats:
                //player posts small blind [$5 USD].
        ...
        static HandAction ParseActionWithoutSize(ReadOnlySpan<char> line, Street street)
        {
            ...
            switch (identifier)
            {
                ...
                case 'd'://"<playername> folds"
                    return new HandAction(line.Remove(line.Length - 6).ToString(), HandActionType.FOLD, 0m, street);
                case 'k'://"<playername> checks"
                    return new HandAction(line.Remove(line.Length - 7).ToString(), HandActionType.CHECK, 0m, street);
                default:
                    throw new ArgumentException("Unknown Action: \"" + line.ToString() + "\"");
            }
        }

        static HandAction ParseActionWithSize(ReadOnlySpan<char> line, Street currentStreet)
        {
            int amountStartIndex = line.LastIndexOf('[');
            decimal amount = ParseDecimal(line.Slice(amountStartIndex + 1));
            ...
            switch (actionID)
            {
                case 't'://Player bets [$23.75 USD]
                    playerName = line.Remove(amountStartIndex - 6).ToString();//" bets ".Length
                    return new HandAction(playerName, HandActionType.BET, amount, currentStreet);
                case 'l'://Player calls [$25 USD]
                    playerName = line.Remove(amountStartIndex - 7).ToString();//" calls ".Length
                    return new HandAction(playerName, HandActionType.CALL, amount, currentStreet);
                case 'e'://Player raises [$30 USD]
                    playerName = line.Remove(amountStartIndex - 8).ToString();//" raises ".Length
                    return new HandAction(playerName, HandActionType.RAISE, amount, currentStreet);
                case 'n'://"dr. spaz is all-In  [$4.90 USD]"
                    playerName = line.Remove(amountStartIndex - 12).ToString();//" is all-In  ".Length
                    return new HandAction(playerName, HandActionType.ALL_IN, amount, currentStreet);
                default:
                    throw new ArgumentException("Unknown actionID: " + line.ToString());
            }
        }

        static WinningsAction ParseWinsAction(ReadOnlySpan<char> line, int potID = 0)
        {
            //Expected Formats:
            //"Player wins $5.18 USD"
            //Tournament format:
            //saboniiplz wins 28,304 chips
            int nameEndIndex = line.IndexOf(" wins ", StringComparison.Ordinal);
            string playerName = line.Remove(nameEndIndex).ToString();

            int amountStartIndex = nameEndIndex + 6;
            decimal amount = ParseDecimal(line.Slice(amountStartIndex));
        ...
        static void ParseDotAction(ReadOnlySpan<char> line, Street street, List<HandAction> actions, List<WinningsAction> winners)
        {
            string playerName;
            ...
            else if (line.Contains(" shows", StringComparison.Ordinal))
            {
                if (isWinType(line, " shows [", ref playerNameIndex))
                {
                    playerName = line.Remove(playerNameIndex).ToString();
                    actions.Add(new HandAction(playerName, HandActionType.SHOW, 0m, Street.Showdown));
                    return;
                }
                else if (line.Contains(" for low.", StringComparison.Ordinal))
                {
                ...
                {
                    throw new ArgumentException("Unknown Showdown: " + line.ToString());
                }
            }
            else if (line.Contains(" wins ", StringComparison.Ordinal))
            {
                playerName = line.Remove(playerNameIndex).ToString();
                int amountStartIndex = line.LastIndexOf('[');
                ...
                if (amountStartIndex == -1)//Wins Side Pot
                {
                    ...
                    if (idStartIndex != -1)
                    {
                        int idEndIndex = line.Slice(idStartIndex + sidePotID.Length).IndexOf(' ');
                        ReadOnlySpan<char> idStr = line.SliceBetween(idStartIndex + sidePotID.Length, idEndIndex);
            ...
            else if (line.Contains(" does not ", StringComparison.Ordinal))
            {
            ...
            else if (line.Contains(" doesn't show", StringComparison.Ordinal))
            {
        ...
        static bool isWinType(ReadOnlySpan<char> line, string type, ref int index)
        {
        ...
        protected override long[] ParseHandId(List<string> header)
        {
            ...
            ReadOnlySpan<char> line = header[0].AsSpan();
            ReadOnlySpan<char> handId = line.SliceBetween(firstDigitIndex, line.Length - 6);
        ...
        private Limit ParseCashgameLimit(List<string> header)
        {
            ...
            ReadOnlySpan<char> line = header[1].AsSpan();
            ReadOnlySpan<char> limitSubstring = line.SliceBetween(1, line.IndexOf(' '));
            ...
            if (limitSubstring.Contains("/", StringComparison.Ordinal))
            {
        ...
        static Limit Parse20BBLimit(ReadOnlySpan<char> limitSubstring, Currency currency)
        {
            decimal bigblind = limitSubstring.ParseAmount() * 0.4m;
            return Limit.FromSmallBlindBigBlind(bigblind / 2, bigblind, currency);
        }

        static Limit ParseNormalLimit(ReadOnlySpan<char> limitSubstring, Currency currency)
        {
            //Expected limitSubstring format:
            //0.05/$0.10
            int splitIndex = limitSubstring.IndexOf('/');
            ReadOnlySpan<char> SB = limitSubstring.Remove(splitIndex);
            ReadOnlySpan<char> BB = limitSubstring.Slice(splitIndex + 2);
        ...
        static Limit ParseBuyInLimit(ReadOnlySpan<char> limitSubstring, Currency currency)
        {
        ...
        static Currency ParseLimitCurrency(ReadOnlySpan<char> line)
        {
            ...
            switch (currencySymbol)
            {
                ...
                default:
                    throw new LimitException(line.ToString(), "Unrecognized currency symbol " + currencySymbol);
            }
        }
        ...
        protected override PlayerList ParsePlayers(List<string> seats)
        {
            ...
            {
                var lineSpan = line.AsSpanParser();
                char lastChar = lineSpan.GetLast();
                ...
                if (playerNameStartIndex > 1 && lastChar == ')' && line.StartsWithFast("Seat "))
                {
                    ...
                    decimal stack = ParseDecimal(line.AsSpan().Slice(openParenIndex + 2));
            ...
            if (heroCardsLine != null)
            {
                ...
                ReadOnlySpan<char> cards = heroCardsLine.AsSpan().SliceBetween(openSquareIndex + 3, heroCardsLine.Length - 2);
                HoleCards holeCards = HoleCards.FromCards(cards);
        ...
        static decimal ParseDecimal(ReadOnlySpan<char> line)
        {
            int endIndex = line.IndexOf(' ');
            if (endIndex == -1)
                endIndex = line.IndexOf(']');

            return line.Remove(endIndex).ParseAmount();
        }

        static Street ParseStreet(ReadOnlySpan<char> line)
        {
